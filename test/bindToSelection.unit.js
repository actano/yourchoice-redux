...
describe('bindToSelection - get API, bound to given selection name', () => {
  ...
  it('should separate states of different name bindings', () => {
    ...
    const state1 = reducer(undefined, boundApiA.actions.setItems(['A', 'B', 'C']))
    const state2 = reducer(state1, boundApiB.actions.setItems(['X', 'Y', 'Z']))
    expect(boundApiA.selectors.getItems(state2)).to.deep.equal(['A', 'B', 'C'])
    expect(boundApiB.selectors.getItems(state2)).to.deep.equal(['X', 'Y', 'Z'])
  })
